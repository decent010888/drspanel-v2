...
class Notifications
{
    ...
    public static function appointmentSmsNotification($appointment_id,$type='',$by=''){
        ...
        $data=array();$notification=array();$user=array();$token='';
        ...
        if($type == 'appointment'){
            ...
            $send_message='Hello '.$appointment->user_name.' your appointment is successfully booked with doctor '.$appointment->doctor_name.' on '.$appointment_date.' at '.$appointment_time.' with token no '.$appointment->token;

            if($by == 'patient'){
                $notify_message='Hello '.$appointment->user_name.' your appointment is successfully booked with doctor '.$appointment->doctor_name.' on '.$appointment_date.' at '.$appointment_time.' with token no '.$appointment->token.'.';
                if($appointment->user_id > 0){
                    $notification_data=['type'=>'appointment','message'=>$notify_message,'user_id'=>$user->id ,'appointment_id'=>$appointment->id];
                    $sendNotification = Notifications::createNotification($user,$token,$type='appointment','DrsPanel Appointment Booking',
                        $notify_message,$notification_id=$appointment_id,
                        $device_type = $user->device_type,$notification_data);
                }
            }
            $sendSms=Notifications::send_sms($send_message,$phones,'No',91,0);
        }
        elseif($type == 'cancelled' && $by == 'patient'){
            $appointment_date=date('d/m/Y',strtotime($appointment->date));
            $appointment_time=date('h:i a',$appointment->appointment_time);
            $send_message='Your appointment with '.$appointment->doctor_name.' on '.$appointment_date.', '.$appointment_time.' with token no '.$appointment->token.'. is successfully cancelled by you';

            $notify_message='Your appointment with '.$appointment->doctor_name.' on '.$appointment_date.', '.$appointment_time.' with token no '.$appointment->token.'. is successfully cancelled by you';
        ...
        elseif($type == 'cancelled' && $by == 'doctor'){
            $appointment_date=date('d/m/Y',strtotime($appointment->date));
            $appointment_time=date('h:i a',$appointment->appointment_time);
            $send_message='Your appointment with '.$appointment->doctor_name.' on '.$appointment_date.', '.$appointment_time.' with token no '.$appointment->token.'. has been cancelled by doctor due to non availability or personal issues';

            $notify_message='Your appointment with '.$appointment->doctor_name.' on '.$appointment_date.', '.$appointment_time.' with token no '.$appointment->token.'. has been cancelled by doctor due to non availability or personal issues';
    ...
    public static function reminderNotification($reminder_id){
        ...
        if(!empty($reminder)){
            ...
            $user=array();$token='';
            $message='Hello '.$appointment->user_name.' your appointment is booked with doctor '.$appointment->doctor_name.', '.'Please reach the clinic/hospital on time.';
            if($appointment->user_id > 0){
                $user=User::findOne($appointment->user_id);
                $token=$user->token;
