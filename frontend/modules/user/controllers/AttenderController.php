...
class AttenderController extends \yii\web\Controller
{
    ...
    public function actionProfileFieldEdit(){
        if(Yii::$app->request->isAjax && Yii::$app->request->post()){
            ...
            if(isset($post['user_id']) && !empty($post['user_id'])){
                ...
                echo $this->renderAjax('/common/_edit_input_popup',['type'=>$post['type'],'user'=>$user,'userType'=>$post['userType']]); exit();
            }
        }
    }
            ...
            if($type == 'email'){
                ...
                if($checkexist){
                    ...
                    if($user->email == $post['identity']){
                        ...
                        echo $this->renderAjax('/common/_verify_otp_popup',['type'=>$post['type'],'user'=>$user,'model'=>$model]); exit();
        ...
        if($parentGroup == Groups::GROUP_HOSPITAL){
            ...
            if(!empty($slug)){
                ...
                if(!empty($doctorProfile)){
                    ...
                    return $this->render('shift/my-hospital-shifts',
                        ['address'=>$address,'shifts'=>$shifts,'doctor'=>$doctor,'hospital'=>$hospital,'doctorProfile'=>$doctorProfile]);
                }
                else{
    ...
    function hospitalAppointment($slug,$type,$parent_id,$id,$date,$hospital,$string){
        $selected_speciality=0; $speciality_check=0;
            ...
            if(!empty($slug)){
                ...
                if(!empty($doctorProfile)) {
                    ...
                    return $this->render('/attender/hospital/current-appointments',
                        ['defaultCurrrentDay' => strtotime($date), 'appointments' => $appointments, 'bookings' => $bookings, 'type' => $type, 'current_shifts' => $current_shifts, 'doctor' => $doctor,'doctorProfile'=>$doctorProfile,'userType'=>'attender']);
        ...
        elseif($type == 'current_shift'){
            ...
            $type='book';
            $current_shifts=0; $slots=array();$message='';
            if(!empty($slug)){
                $doctorProfile=UserProfile::find()->where(['slug'=>$slug])->one();
                if(!empty($doctorProfile)){
                    $doctor=User::findOne($doctorProfile->user_id);
                    ...
                    return $this->render('/attender/hospital/appointments',
                        ['defaultCurrrentDay'=>strtotime($date),'appointments'=>$appointments,'slots'=>$slots,'type'=>$type,'current_shifts'=>$current_shifts,'doctor'=>$doctor,'doctorProfile'=>$doctorProfile,'userType'=>'attender','message'=>$message]);
    ...
    function doctorAppointment($type,$parent_id,$id,$date,$doctor,$string){
        ...
        elseif($type == 'current_shift'){
            ...
            return $this->render('/attender/doctor/appointments',
                ['defaultCurrrentDay'=>strtotime($date),'appointments'=>$appointments,'slots'=>$slots,'type'=>$type,'current_shifts'=>$current_shifts,'doctor'=>$doctor,'message'=>$message]);
        }
    }
    ...
    public function actionAjaxAppointment(){
        ...
        if(Yii::$app->request->post() && Yii::$app->request->isAjax){
            ...
            echo $this->renderAjax('/common/_bookings',['bookings'=>$bookings,'doctor_id'=>$doctor_id,'userType'=>'attender']); exit();
        }
    }
            ...
            if ($status == 'next' || $status == 'skip') {
                ...
                if(!empty($checkFirstAppointment)){
                    ...
                    if(!empty($checkFirstAppointment)){
                        ...
                        Yii::$app->session->setFlash('error', "'Appointments are not paid'");           return $this->redirect(Yii::$app->request->referrer);
                    }
    ...
    public function actionGetAppointmentDetail(){
        if(Yii::$app->request->post() && Yii::$app->request->isAjax){
            ...
            echo $this->renderAjax('/common/_booking_detail',['booking'=>$booking,'doctor_id'=>$appointment->doctor_id,'userType'=>'attender','booking_type' => $booking_type,'current'=>$current]); exit();
        }
    }
    ...
    public function actionPatientHistory($slug=''){
        ...
        $current_selected=0;$checkForCurrentShift=0;
    ...
    public function actionAjaxHistoryContent(){
        ...
        if(Yii::$app->request->isAjax && Yii::$app->request->post()){
            ...
            echo $this->renderAjax('/attender/history-statistics/_history-content',['history_count'=>$history,'typeCount'=>$typeCount,'appointments'=>$appointments,'shifts'=>$shiftAll,'doctor'=>$doctor,'current_selected'=>$current_selected]); exit;
        }
        echo 'error'; exit;
    }
    ...
    public function actionAjaxHistoryAppointment() {
        ...
        if(Yii::$app->request->post() && Yii::$app->request->isAjax){
            ...
            echo $this->renderAjax('/attender/history-statistics/_history-patient',['appointments'=>$bookings,'doctor_id'=>$doctor_id,'userType'=>'attender','history_count'=>$history,'typeCount'=>$typeCount,]); exit();
        }
    }
    ...
    public function actionUserStatisticsData($slug=''){
        ...
        if($parentGroup == Groups::GROUP_HOSPITAL){
            ...
            if($speciality_check == 1){
                ...
                return $this->render('/common/speciality-doctor-list',['defaultCurrrentDay'=>strtotime($date),'data_array'=>$data_array,'hospital'=>$hospital,'selected_speciality'=>$selected_speciality,'type'=>$type,'actionType'=>'user_history','userType'=>'attender','page_heading' => 'User Statistics Data']);
            }
            else{
                throw new NotFoundHttpException('The requested page does not exist.');
            }
