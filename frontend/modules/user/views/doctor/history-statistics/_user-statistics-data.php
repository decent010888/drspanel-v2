...
$getTabContent="'ajax-statistics-data'";
$js="
    $(document).on('click', '.get-appointments',function () {        
        ...
        $.ajax({
          method:'POST',
          url: $getTabContent,
          data: {type:type,date:date,shift_id:schedule_id}
        })
...
?>
  ...
  <?php if(count($shifts)>0){ ?>
    <div class="doc-timingslot">
          <ul>
              <?php echo $this->render('/common/_shifts',['shifts'=>$shifts,'current_shifts'=>$current_shifts,'doctor'=>$doctor,'type'=>'user_history','userType'=>'doctor']);?>
          </ul>
      </div>
